...
describe('Bridge peripherals layer 2', () => {
  ...
  before(async function () {
    ...
    const StandardMETAERC20 = await ethers.getContractFactory('StandardMETAERC20')
    const standardMETAERC20Logic = await StandardMETAERC20.deploy()

    const UpgradeableBeacon = await ethers.getContractFactory(
      'UpgradeableBeacon'
    )

    const beacon = await UpgradeableBeacon.deploy(standardMETAERC20Logic.address)
    ...
    const METASysMock = await ethers.getContractFactory('METASysMock')
    const METAsysmock = await METASysMock.deploy()
    await network.provider.send('hardhat_setCode', [
      '0x0000000000000000000000000000000000000064',
      await network.provider.send('eth_getCode', [METAsysmock.address]),
    ])
  ...
  it('should deploy erc20 tokens correctly', async function () {
    ...
    const Erc20 = await ethers.getContractFactory('StandardMETAERC20')
  ...
  it('should mint erc20 tokens correctly', async function () {
    ...
    const dest = sender
    const amount = '1'
    const initializeData = encodeTokenInitData('METAToken', 'ATKN', '18')
