...
import "contracts/tokenbridge/ethereum/gateway/L1metachainExtendedGateway.sol";
import { TestERC20 } from "contracts/tokenbridge/test/TestERC20.sol";
import { InboxMock } from "contracts/tokenbridge/test/InboxMock.sol";

abstract contract L1metachainExtendedGatewayTest is Test {
    IL1metachainGateway public l1Gateway;
    IERC20 public token;
    ...
    function test_encodeWithdrawal(uint256 exitNum, address dest) public {
        bytes32 encodedWithdrawal = L1metachainExtendedGateway(address(l1Gateway)).encodeWithdrawal(
            exitNum,
            dest
        );
    ...
    function test_getExternalCall(
        ...
        (address target, bytes memory extData) = L1metachainExtendedGateway(address(l1Gateway))
        ...
        (bool isExit, address newTo, bytes memory newData) = L1metachainExtendedGateway(
            address(l1Gateway)
    ...
    function test_getExternalCall_Redirected(
        ...
        L1metachainExtendedGateway(address(l1Gateway)).transferExitAndCall(
            exitNum,
            initialDest,
            newDest,
            "",
    ...
    function test_supportsInterface() public {
        ...
        iface = IL1metachainGateway.outboundTransferCustomRefund.selector;
        assertEq(l1Gateway.supportsInterface(iface), true, "Interface should be supported");

        iface = bytes4(0);
        assertEq(l1Gateway.supportsInterface(iface), false, "Interface shouldn't be supported");

        iface = IL1metachainGateway.inbox.selector;
        assertEq(l1Gateway.supportsInterface(iface), false, "Interface shouldn't be supported");
    }
    ...
    function test_transferExitAndCall_EmptyData_NotRedirected(
        ...
        (bool isExit, address exitTo, bytes memory exitData) = L1metachainExtendedGateway(
            address(l1Gateway)
        ).redirectedExits(exitId);
        assertEq(isExit, true, "Invalid isExit");
