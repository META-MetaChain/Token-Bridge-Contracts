...
import { L2GatewayRouter } from "contracts/tokenbridge/metachain/gateway/L2GatewayRouter.sol";
...
contract L1OrbitGatewayRouterTest is L1GatewayRouterTest {
    ...
    function test_setGateway_revert_NotMETAEnabled() public override {
        address nonMETAEnabledToken = address(new ERC20("X", "Y"));
        vm.deal(nonMETAEnabledToken, 100 ether);
        vm.mockCall(
            nonMETAEnabledToken,
            abi.encodeWithSignature("ismetachainEnabled()"),
    ...
    function test_setGateway_revert_NotToContract() public override {
        ...
        vm.mockCall(token, abi.encodeWithSignature("ismetachainEnabled()"), abi.encode(uint8(0xb1)));
