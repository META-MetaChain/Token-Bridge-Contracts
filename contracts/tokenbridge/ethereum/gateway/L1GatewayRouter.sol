...
import { metachainEnabledToken } from "../ICustomToken.sol";
import "../L1metachainMessenger.sol";
import "../../libraries/gateway/GatewayRouter.sol";
import "../../metachain/gateway/L2GatewayRouter.sol";
import "../../libraries/ERC165.sol";
import "./IL1GatewayRouter.sol";
import "./IL1metachainGateway.sol";

/**
 * @title Handles deposits from Erhereum into metachain. Tokens are routered to their appropriate L1 gateway (Router itself also conforms to the Gateway itnerface).
 * @notice Router also serves as an L1-L2 token address oracle.
 */
contract L1GatewayRouter is
    WhitelistConsumer,
    L1metachainMessenger,
    GatewayRouter,
    ERC165,
    IL1GatewayRouter
{
    ...
    function _setGatewayWithCreditBack(
        ...
        require(
            metachainEnabledToken(msg.sender).ismetachainEnabled() == uint8(0xb1),
            "NOT_META_ENABLED"
        );
    ...
    /**
     * @notice Deposit ERC20 token from Ethereum into metachain using the registered or otherwise default gateway
    ...
    function outboundTransferCustomRefund(
        ...
        // here we use `IL1metachainGateway` since we don't assume all ITokenGateway implements `outboundTransferCustomRefund`
        return
            IL1metachainGateway(gateway).outboundTransferCustomRefund{ value: msg.value }(
                _token,
                _refundTo,
                _to,
                _amount,
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        returns (bool)
    {
        // registering interfaces that is added after META-bridge-peripherals >1.0.11
